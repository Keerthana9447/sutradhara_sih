...
## Six flagship features (beyond the core classify → route → retrieve → cite pipeline)

1. **Dynamic multi-hop Knowledge Graph** (`app/graph_reasoning.py`, `/api/graph/reason`) — traced fresh from the live corpus for the chosen category/jurisdiction on every call, including an optional export-readiness branch kept in a separate, jurisdiction-isolated sub-graph. Not a static diagram.
2. **Live Evaluation Benchmark** (`app/eval_runner.py`, `/api/eval/benchmark`) — runs a 20-item labeled test set through the actual running app and reports freshly computed classification/abstention/clarification accuracy and citation hit rate, plus two hard invariants (jurisdiction-isolation violations, citation-integrity violations) that must read zero.
...
4. **TKDL / prior-art resemblance scoring** (`app/tkdl_similarity.py`) — TF-IDF similarity against a small curated set of well-known classical formulations, quantifying "how close is this to known traditional knowledge" rather than only pointing at TKDL.
5. **IP Posture Summary PDF export** (`app/posture_pdf.py`, `/api/posture-pdf`) — turns one analysis into a downloadable, timestamped PDF, rendered from exactly what the UI already showed.
...
- The corpus is a curated prototype set (29 documents — 15 India + 14
  ...
  GRATK Treaty), not the full legal
